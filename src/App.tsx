...
function App() {
  ...
  return (
    <div style={{ padding: "5px" }}>
      <Space>
        <Button type='primary' onClick={() => { createCommand() } }>生成命令</Button>
        <Button>读取配置</Button>
        <Button>加载配置</Button>
      </Space>
      <Collapse className="comp_list" bordered={false} lazyload={false}>
        <Collapse.Item name='1' header="数据">
          ...
          <CmpFolder ref={refs[2]} id='train_data_dir' title="训练集" defaultPath={"D:\\LoraTrainData\\trains\\"} ></CmpFolder>
          <CmpFolder ref={refs[3]} id='output_dir' title="输出路径" defaultPath={"D:\\LoraTrainData\\output\\"} ></CmpFolder>
          <CmpText ref={refs[4]} id='output_name' title='输出名称' ></CmpText>
          <CmpNum ref={refs[5]} id='save_every_n_epochs' title='Epoch保存间隔' defaultValue={1} min={1} max={30} step={1} precision={0} ></CmpNum>
        </Collapse.Item>
        ...
        <Collapse.Item name='2' header="不常用的设置">
          ...
          <CmpText ref={refs[10]} id='save_model_as' title='SaveModelAs' defaultValue='safetensors' enable={false} ></CmpText>
        ...
        <Collapse.Item name='3' header="基本设置">
          <CmpText ref={refs[15]} id='resolution' title="分辨率" defaultValue="512,512"></CmpText>
          <CmpCombox ref={refs[16]} id='optimizer_type' title="优化器" defaultValue='Lion' options={optTypes}></CmpCombox>
          <CmpText ref={refs[17]} id='network_module' title="网格类型" defaultValue='networks.lora' enable={false}></CmpText>
          <CmpNum ref={refs[18]} id='max_train_epochs' title='Epoch' defaultValue={10} min={1} max={300} step={1} precision={0}></CmpNum>
          <CmpNum ref={refs[19]} id='learning_rate' title='学习率' defaultValue={0.00001} min={0.000001} max={0.001} step={0.000001} precision={6}></CmpNum>
          <CmpNum ref={refs[20]} id='unet_lr' title='UNetLr' defaultValue={0.00001} min={0.000001} max={0.001} step={0.000001} precision={6}></CmpNum>
          ...
          <CmpNum ref={refs[24]} id='train_batch_size' title='训练批次' defaultValue={1} min={1} max={5} step={1} precision={0}></CmpNum>
        </Collapse.Item>

        <Collapse.Item name='4' header="学习策略">
          <CmpCombox ref={refs[25]} id='lr_scheduler' title="学习策略" defaultValue='cosine_with_restarts' options={lrSchedulers}></CmpCombox>
          <CmpNum ref={refs[26]} id='lr_scheduler_num_cycles' title='学习周期数' defaultValue={4} isOptional={true} min={1} max={30} step={0} precision={0} ></CmpNum>
        </Collapse.Item>
        ...
        <Collapse.Item name='7' header="增强参数">
          ...
          <CmpNum ref={refs[35]} id='prior_loss_weight' title='prior_loss_weight' enable={false} defaultValue={1} isOptional={true} min={0} max={1} step={0.01} precision={2} ></CmpNum>
          <CmpNum ref={refs[36]} id='min_snr_gamma' title='min_snr_gamma' enable={false} defaultValue={5} isOptional={true} min={0} max={10} step={1} precision={0} ></CmpNum>
        </Collapse.Item>
      </Collapse>
