...
function App() {
  ...
  const createCommand_xl = async () => {
    var cmd = `${sdHomeRef.current?.getString()}\\venv\\Scripts\\python.exe -m accelerate.commands.launch --num_cpu_threads_per_process=8 ${sdHomeRef.current?.getString()}\\sdxl_train_network.py `;
    refs.map((ref) => {
      if (ref.current != null) {
        var arg_str = ref.current.getArgumentString();
        if (arg_str.length > 0) {
    ...
    <div style={{ padding: '5px'}}>
      <Grid.Row>
        ...
        <Collapse className="comp_list" bordered={true} lazyload={false} style={{width: '100%', overflow: 'hidden' }} defaultActiveKey={['0']} >
          ...
          <Collapse.Item name="1" header="输出&日志">      
          ...
          <Collapse.Item name="2" header="训练设置">            
